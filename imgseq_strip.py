...
if len(sys.argv) < 2:
    sys.stderr.write('Usage: imgseq_strip.py /path/to/exr/folder\n')
    exit(-10)
...
try:
    ...
    g_plate_transcode_url = config.get('scan_ingest', 'imgseq_strip_url')
    g_imgseq_regexp = re.compile(config.get(g_ih_show_code, 'imgseq_regexp'))
except:
    g_plate_transcode_url = 'http://glenrothes.qppe.tech/vfxbot/imgseq_strip'
...
if response.status_code != 200:
    sys.stderr.write('Error: Errors occurred while attempting to strip the image sequence.\n')
    ...
    sys.stdout.write('Stripped image sequence path: %s\n' % destination_filepath)
    exit()
